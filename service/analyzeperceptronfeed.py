...
class TwitterFeed():
    def __init__(self,bad,good,spam,track,stop,user):
        ...
        self.stream = tweepy.Stream(auth, TwitterListener(bad,good,spam,stop,user),timeout=30)
...
def saveTweet(tipo, tweet,texto,guess,user):
    ...
    if(tipo == 'pos'):
        ...
        db[board].goodinfo.insert_one({"_id": auxcount , "id_str":id_str , "n_followers":n_followers , "location": location , "name": name , "friends_count": friends_count , "time": timenow, "tweet":tweet,"guess":guess})

    elif(tipo == 'neg'):
        db[board].badcounter.update({"type":'contagem'},{"$inc": {'count':1}},upsert=True)
        auxcount = db[board].badcounter.find_one()
        auxcount = auxcount["count"]
