...
urls= ( '/InstantFeed','InstantFeed',
        ...
        '/getInfoTweetsSpam','getInfoTweetsSpam',
        '/getInfoTweetsBad','getInfoTweetsBad',
...
class StartFeedModifie:
    def GET(self):
        ...
        if countbad != None:
            ...
            countbad=0
        countspam = db[auxuser].spamcounter.find_one()
        if countspam != None:
            countspam = int(countspam["count"])
        else:
            countspam=0
        if ((countbad+countgood+countspam)< number):
            send[0]='false'
        else:
            send[0]='true'
        send = json.dumps(send)
...
class StartFeedPerceptron:
    def GET(self):
        ...
        db.LOGIN.login.update({"_id":int(data['4'])},{"$set": {"status":"true"}},upsert=True)
        self.aux.start()
        db.LOGIN.login.update({"_id":int(data['4'])},{"$set": {"status":"false"}},upsert=True)
        send = {}
	send[0]='true'
	return json.dumps(send)
...
class getInfoTweetsGood:
    def GET(self):
        ...
        for line in query:
            ...
            send[count]['tweet']=line['tweet']
            send[count]['guess']=line['guess']

            count+=1
        send = json.dumps(send)
        return send

class getInfoTweetsSpam:
    def GET(self):
        web.header('Access-Control-Allow-Origin',      '*')
        web.header('Access-Control-Allow-Credentials', 'true')
        web.header('Content-Type', 'application/json')
        ...
        auxuser = 'BOARD'+user
        self.db=pymongo.MongoClient()
        query = self.db[auxuser].spaminfo.find()
        for line in query:
            send[count]={}
            send[count]['id']=line['_id']
            send[count]['n_followers']=line['n_followers']
