...
class TwitterListener(tweepy.StreamListener):
    ...
    def on_data(self, data):
        ...
        if 'text' in tweet:
            ...
            guess3= help(self.bad3,self.good3,self.spam3,tweet['text'])
            ...
            if totalguess>0: #and (guess2=='pos' or guess3=='pos'):
                ...
                saveTweet('pos',tweet,texto,totalguess,self.user)
                ...
                saveTweet('neg',tweet,texto,totalguess,self.user)
                ...
                texto = self.user+'-'+texto
                self.producer.send(self.topicbad,texto)
                saveTweet('spam',tweet,texto,totalguess,self.user)
...
def saveTweet(tipo,tweet,texto,totalguess,user):
    db = pymongo.MongoClient()
    aux = 'BOARD'+user
    board = db[aux]
    ...
    if(tipo == 'pos'):
        ...
        board.goodinfo.insert_one({"_id": auxcount , "id_str":id_str , "n_followers":n_followers , "location": location , "name": name , "friends_count": friends_count, "time":timenow , "tweet":texto , "guess":totalguess})

    elif(tipo == 'neg'):
        board.badcounter.update({"type":'contagem'},{"$inc": {'count':1}},upsert=True)
        auxcount = board.badcounter.find_one()
        auxcount = auxcount["count"]
        board.badinfo.insert_one({"_id": auxcount , "id_str":id_str , "n_followers":n_followers , "location": location , "name": name , "friends_count": friends_count, "time":timenow, "tweet":texto , "guess":totalguess})

    elif(tipo == 'spam'):
        board.spamcounter.update({"type":'contagem'},{"$inc": {'count':1}},upsert=True)
        auxcount = board.spamcounter.find_one()
        auxcount = auxcount["count"]
        board.spaminfo.insert_one({"_id": auxcount , "id_str":id_str , "n_followers":n_followers , "location": location , "name": name , "friends_count": friends_count, "time":timenow, "tweet":texto, "guess":totalguess})
